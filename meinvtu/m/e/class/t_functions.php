...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	$firststr='<li class="hide"><span class="pageinfo hide" id="pageinfo" pageinfo="'.$totalpage.'">共 <strong>'.$totalpage.'</strong>页<strong>'.$num.'</strong>条</span></li>';
	//上一页
	if($page<>1)
	{
		//$toppage='<li><a href="'.$dolink.$add['dofile'].$type.'">'.$fun_r['startpage'].'</a>&nbsp;';
		$pagepr=$page-1;
		if($pagepr==1)
		{
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1='<li class="thisclass" id="thisclass" thisclass="'.$i.'"><a>';
			$is_2="</a></li>";
		}
		elseif($i==1)
		{
			$is_1='<li class="hide"><a href="'.$dolink.$add['dofile'].$type.'">';
			$is_2="</a></li>";
		}
		else
		{
			$thispagelink=$repagenum&&$repagenum<$i?eReturnRewritePageLink2($add,$i):$dolink.$add['dofile'].'_'.$i.$type;
			$is_1='<li class="hide"><a href="'.$thispagelink.'">';
			$is_2="</a></li>";
		}
...
function sys_ShowTextPage($totalpage,$page,$dolink,$add,$type,$search=""){
	...
	$snum=2;
	//$totalpage=ceil($num/$line);//取得总页数
	$firststr='<li id="pageinfo" pageinfo="'.$totalpage.'"  class="pageAll"><a>共'.$totalpage.'页: </a></li>';
	$prepage='<li id="pl"><a href=#>上一页</a></li>';
	//上一页
	if($page<>1)
	{
		...
		$prepage='<li id="pl"><a href="'.$dolink.$prido.'">'.$fun_r['pripage'].'</a></li>';
	}
	//下一页
	$nextpage='<li id="pl"><a href=#>'.$fun_r['nextpage'].'</a></li>';
	if($page!=$totalpage)
	{
		$pagenex=$page+1;
		$nextpage='<li><a href="'.$dolink.$add[filename].'_'.$pagenex.$type.'">'.$fun_r['nextpage'].'</a></li>';
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		elseif($i==1)
		{
			$is_1='<li class="pageAll"><a href="'$add[filename].$type.'">';
			$is_2="</a></li>";
		}
		else
		{
			$is_1='<li class="pageAll"><a href="'$add[filename].'_'.$i.$type.'">';
			$is_2="</a></li>";
		}
		$returnstr.=$is_1.$i.$is_2;
	}
