...
function tagpage($num,$line,$page_line,$start,$page,$search){
	global $fun_r;
	if($num<=$line)
	{
		return '';
	...
	$returnstr=$firststr.$toppage.$prepage.$nextpage.$lastpage;
	return $returnstr;
}


function starpage($num,$line,$page_line,$start,$page){
	global $fun_r;
	if($num<=$line)
	{
		return '';
...
$infozm=$_GET[infozm];
	...
	for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1='<li class="thisclass" id="thisclass" thisclass="'.($i+1).'"><a>';
			$is_2="</a></li>";
		}
		else
		{
			$is_1='<li class="hide"><a href="'.$url.'-'.$i.$search.'">';
			$is_2="</a></li>";
		}
		$pagenum=$i+1;
		$returnstr.=$is_1.$pagenum.$is_2;
