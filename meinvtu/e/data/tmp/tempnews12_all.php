...
<link href="/css/article_articleV2_tx.css" type="text/css" rel="stylesheet" />
...
<div class="warp yh">
  <div class="hr10"></div>
  <div class="warp"> 当前位置 ：<?=$grurl?> >  <strong><a href="" title="<?=$ecms_gr[title]?>"><?=$ecms_gr[title]?></a></strong> </div>
  <div class="hr10"></div>
  <div class="w960 l">
 ...
 &nbsp;&nbsp;&nbsp;<small>点击:</small><script src=localhoste/public/ViewClick/?classid=<?=$ecms_gr[classid]?>&id=<?=$ecms_gr[id]?>&addclick=1></script>次</div>
    ...
    <div class="hr10"></div>
    <div class="articleV2Body" id="picBody">
<?php
$bqno=0;
...
?>
<?
$preurl="http://$public_r[add_www_92game_net_www]$bqsr[titleurl]";
$pre='true'; 
?>
<?php
}
...
?>
<?
$nexturl="http://$public_r[add_www_92game_net_www]$bqsr[titleurl]";
$next='true'; 
?>
<?php
}
...
?> 
<?
$pd_record=explode(chr(10),$navinfor[morepic]);
for($i=0;$i<count($pd_record);$i++)
{ 
 if($pd_record[$i])
 {
$pd_pic=explode("::::::",$pd_record[$i])
?>
<p align="center">
	<div id='mouse' class='PicHover'><div class='leftCur' onclick='window.open("<?=$preurl?>" ,"_top")'></div><div class='rightCur' onclick='window.open("<?=$nexturl?>" ,"_top")'></div><a href='<?=$grtitleurl?>' class="both"><img alt="<?=$ecms_gr[title]?>" src="<?=$public_r['add_www_92game_net_file']?><?=$pd_pic[1]?>" /></a></div></p>
<?}
}
?>
</div>
    <div class="hr10"></div>
    <ul class="articleV2Page">
    </ul>
...
$bqno++;
?>
        <li><a href="<?=$bqsr['titleurl']?>" class="autoPic"><span><img src="<?=$public_r['add_www_92game_net_file'].$bqr['titlepic']?>" width="170"></span></a><a href="<?=$bqsr['titleurl']?>" class="autoTit"><?=$bqr['title']?></a></li>
<?php
}
}
?>
...
<?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('select title,titleurl,newstime,classid from [!db.pre!]ecms_news where isgood<>0 Union All select title,titleurl,newstime,classid from [!db.pre!]ecms_photo where isgood<>0 order by newstime desc limit 16',0,24,0);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
        <li><a href="/<?=$class_r[$bqr[classid]]['classpath']?>/" title="<?=$class_r[$bqr[classid]]['classname']?>">【<?=$class_r[$bqr[classid]]['classname']?>】</a><a href="<?=$bqsr['titleurl']?>"><?=$bqr['title']?></a></li>
<?php
}
}
?>
...
<?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq(11,10,0,0,'','rand()');
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
...
?>
  ...
  <div class="r w220">
    <div class="slideTit"> <strong class="tc ico-vm">安卓APP下载</strong> </div>
    <img src="/images/2vm.png" width="220" />
    <div class="hr101"></div>
	<div class="slideTit"> <strong class="tc ico-vm ico-vm-iphone">苹果IOS下载</strong> </div>
    <img src="/images/iphone2vm.png" width="220" />
    <div class="hr101"></div>
...
$bqno++;
?>
<li><a href="<?=$bqsr['titleurl']?>"><img src="<?=$public_r['add_www_92game_net_file'].$bqr['titlepic']?>" width="220" /><span><?=$bqr['title']?></span></a></li>
<?php
}
}
?>
...
<?
if ($classid==3){
$sql='select tagid,tagname from www_92game_net_enewstags where cid in(1,2) order by rand() limit 20';
}
if ($classid==4){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=4 order by rand() limit 20';
}
if ($classid==5){
$sql='select tagid,tagname from www_92game_net_enewstags where cid in(5,6,7,12,13,14) order by rand() limit 20';
}
if ($classid==7){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=10 order by rand() limit 20';
}
if ($classid==12){
$sql='select tagid,tagname from www_92game_net_enewstags where cid in(15,16,17) order by rand() limit 20';
}
if ($classid==13){
$sql='select tagid,tagname from www_92game_net_enewstags where cid in(15,16,17) order by rand() limit 20';
}
if ($classid==17){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=26 order by rand() limit 20';
}
if ($classid==20){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=25 order by rand() limit 20';
}
if ($classid==22){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=21 order by rand() limit 20';
}
if ($classid==23){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=23 order by rand() limit 20';
}
if ($classid==25){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=22 order by rand() limit 20';
}
if ($classid==26){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=24 order by rand() limit 20';
}
if ($classid==30){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=28 order by rand() limit 20';
}
if ($classid==31){
$sql='select tagid,tagname from www_92game_net_enewstags where cid=29 order by rand() limit 20';
}
if (!$sql){
$sql='select tagid,tagname from www_92game_net_enewstags order by rand() limit 20';
}
	$tagsql=$empire->query($sql);

				while($r=$empire->fetch($tagsql))
{
	?><dd><a href="/tag/<?=$r[tagid]?>.html" target="_blank"><?=$r[tagname]?></a></dd><?}?>
</dl><div class="hr10"></div>
    <div class="hr10"></div><div class="slideTit"><h2>精彩专题</h2></div><ul class="sidePicList">
<?
$zt=$empire->query("select ztname,ztpath from {$dbtbpre}enewszt order by rand() limit 2");
while($r=$empire->fetch($zt))
{
?> 
<li><a href="/<?=$r[ztpath]?>/"><img src="<?=$public_r['add_www_92game_net_file'].$r[ztimg]?>" width="220"><span class="tc"><?=$r[ztname]?></span></a></li>
<?
}
?>
</ul><div class="hr10"></div> </div>
</div>
<div class="footer">
	<div class="warp">
