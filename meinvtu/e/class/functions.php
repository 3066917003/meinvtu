...
function GetHtml($classid,$id,$add,$ecms=0,$doall=0){
	...
	if($add[newstext])
	{
		$add[newstext]=str_replace('/d/',$public_r[add_www_92game_net_file].'/d/',$add[newstext]);
	...
	if ($mid==3&&$add[newstempid]==3||$mid==3&&$add[newstempid]==11)
	{
	$expage="\r\n";//分页符
	}else{
	$expage='[!--empirenews.page--]';//分页符
		...
		for($j=1;$j<=$thispagenum;$j++)
		{
			...
			else
			{
				...
				//$ti_r=explode('[/!--empirenews.page--]',$n_r[$k]);
					...
					if ($mid==1){$ti_r=explode('[/!--empirenews.page--]',$n_r[$k]);}
...
if ($mid==3&&$add[newstempid]==3||$mid==3&&$add[newstempid]==11)
	{
			$pd_record=explode(chr(10),$add[morepic]);
$pd_field=explode("::::::",$pd_record[$k]);
$piclist=$pd_field[1];
            $string=str_replace('[!--piclist--]',$piclist,$string);
	}
			...
			$string=str_replace('[!--pnum--]',$pnum,$string);
...
$piclist=$pd_field[1];
		...
		$string=str_replace('[!--pnum--]','',$string);
		//写文件
		WriteFiletext($file,$addlevel.$string);
	}
	//设为已生成
...
function ReturnAddF($add,$modid,$userid,$username,$do=0,$rdata=0,$ch=0){
	...
	if(empty($do))//增加
	{
		...
		for($i=1;$i<$count;$i++)
		{
			...
			elseif($f=="grjs")
			{
				$value=Returngrjs($add['jtitle'],$add['jimg'],$add['grjs'],$add['delpathid'],$add['pathid'],$add,0);
			}
	...
	elseif($do==1)//修改
	{
		...
		for($i=1;$i<$count;$i++)
		{
			...
			elseif($f=="grjs")
			{
				$value=Returngrjs($add['jtitle'],$add['jimg'],$add['grjs'],$add['delpathid'],$add['pathid'],$add,1);
			}
			elseif($f=="smalltext")//简介
			{
				if(!trim($value))
...
function Returngrjs($jtitle,$jimg,$grjs,$delpathid,$pathid,$add,$down=0){
	$f_exp="::::::";
	$r_exp="@@@@@";
	$returnstr="";
	//增加
	if(empty($down))
	{
		...
		{
			//替换非法字符
			$name=str_replace($f_exp,"",$jtitle[$i]);
			$name=str_replace($r_exp,"",$name);
			$img=str_replace($f_exp,"",$jimg[$i]);
			$img=str_replace($r_exp,"",$img);
			$info=str_replace($f_exp,"",$grjs[$i]);
			$info=str_replace($r_exp,"",$info);
			if($name&&$info&&$img)
			{$returnstr.=$name.$f_exp.$img.$f_exp.$info.$r_exp;}
		}
	}
	//修改
	else
	...
	//去掉最后的字符
	$returnstr=substr($returnstr,0,strlen($returnstr)-5);
	return $returnstr;
}
