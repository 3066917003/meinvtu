...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	if($page<>1)
	{
		$toppage='<li><a href="'.$add['dofile'].$type.'">'.$fun_r['startpage'].'</a>&nbsp;';
		$pagepr=$page-1;
		if($pagepr==1)
		{
		...
		$prepage='<li><a href="'.$prido.'">'.$fun_r['pripage'].'</a></li>';
	}
	//下一页
	if($page!=$totalpage)
	{
		$pagenex=$page+1;
		$nextpagelink=$repagenum&&$repagenum<$pagenex?eReturnRewritePageLink2($add,$pagenex):$add['dofile'].'_'.$pagenex.$type;
		$lastpagelink=$repagenum&&$repagenum<$totalpage?eReturnRewritePageLink2($add,$totalpage):$add['dofile'].'_'.$totalpage.$type;
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1='<li class="thisclass" id="thisclass"><a>';
			$is_2="</a></li>";
		}
		elseif($i==1)
		{
			$is_1='<li><a href="'.$add['dofile'].$type.'">';
			$is_2="</a></li>";
		}
		else
		{
			$thispagelink=$repagenum&&$repagenum<$i?eReturnRewritePageLink2($add,$i):$add['dofile'].'_'.$i.$type;
...
function sys_ShowTextPage($totalpage,$page,$dolink,$add,$type,$search=""){
	...
	if($page<>1)
	{
		...
		$prepage='<li id="pl"><a href="'.$prido.'">'.$fun_r['pripage'].'</a></li>';
	}
	//下一页
	if($page!=$totalpage)
	{
		$pagenex=$page+1;
		$nextpage='<li id="nl"><a href="'.$add[filename].'_'.$pagenex.$type.'">'.$fun_r['nextpage'].'</a></li>';
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1='<li class="thisclass"><a>';
			$is_2="</a></li>";
		}
		elseif($i==1)
		{
			$is_1='<li><a href="'.$add[filename].$type.'">';
			$is_2="</a></li>";
		}
		else
		{
			$is_1='<li><a href="'.$add[filename].'_'.$i.$type.'">';
			$is_2="</a></li>";
		}
		$returnstr.=$is_1.$i.$is_2;
	}
